...
class DbOperation
{
    ...
    public function getAllTokens_asd(){
		$stmt = $this->con->prepare("SELECT token FROM devices WHERE login like 'asd' ");
        $stmt->execute(); 
	 ...
	 public function getAllTokens_dunot(){
		$stmt = $this->con->prepare("SELECT token FROM devices WHERE login like 'Dunot' ");
        $stmt->execute(); 
        $result = $stmt->get_result();
        $tokens = array(); 
        while($token = $result->fetch_assoc()){
	...
	public function getAllTokens_home(){
		$stmt = $this->con->prepare("SELECT token FROM devices WHERE login like 'home' ");
        $stmt->execute(); 
        $result = $stmt->get_result();
        $tokens = array(); 
        while($token = $result->fetch_assoc()){
